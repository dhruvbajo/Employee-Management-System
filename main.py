...
def save():
    ...
    if len(name) == 0 or len(address) == 0 or len(pan) == 0 or len(dob) == 0 or len(age) == 0:
        ...
        if is_ok:
            with open("data.csv", "a") as data_file:
                data_file.write(f"{name},{gender},{mobile},{address},{pan},{dob},{age},{email}\n")
                name_input.delete(0, END)
                mobile_input.delete(0, END)
                address_input.delete(0, END)
                pan_input.delete(0, END)
...
pan_label = Label(text="PAN CARD No.:")
pan_label.grid(column=0, row=6, pady=10)
...
pan_input = Entry(width=50)
pan_input.grid(column=1, row=6, columnspan=2)
